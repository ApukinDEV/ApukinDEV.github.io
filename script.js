...
function switchTurn() {
    ...
    if (isInCheck(currentPlayer)) {
        if (isCheckmate(currentPlayer)) {
            ...
            const square = document.querySelector(`[data-row='${findKing(currentPlayer).row}'][data-col='${findKing(currentPlayer).col}']`);
                if (square) {
...
function endGame(winner) {
    const square = document.querySelector(`[data-row='${findKing(currentPlayer).row}'][data-col='${findKing(currentPlayer).col}']`);
                if (square) {
                    square.classList.add('checkmated');
                }
    gameEnded = true;
    clearInterval(timerInterval);
}
...
function isValidMove(fromRow, fromCol, toRow, toCol, lookingForCheck) {
    ...
    switch (piece.toLowerCase()) {
        ...
        case 'k': return isValidKingMove(fromRow, fromCol, toRow, toCol, false);
...
function isValidKingMove(fromRow, fromCol, toRow, toCol, check) {
    const rowDiff = Math.abs(fromRow - toRow);
    const colDiff = Math.abs(fromCol - toCol);
        ...
        for (let col = 0; col < 8; col++) {
            ...
            if ((opponent === 'white' && piece === piece.toUpperCase()) || (opponent === 'black' && piece === piece.toLowerCase())) {
                if (piece.toLowerCase() === 'p') {
                    if (isValidPawnMove(row, col, kingPos.row, kingPos.col, piece)) return true;
                } else if (piece.toLowerCase() === 'n') {
                    if (isValidKnightMove(row, col, kingPos.row, kingPos.col)) return true;
                } else if (piece.toLowerCase() === 'b') {
                    if (isValidBishopMove(row, col, kingPos.row, kingPos.col)) return true;
                } else if (piece.toLowerCase() === 'r') {
                    if (isValidRookMove(row, col, kingPos.row, kingPos.col)) return true;
                } else if (piece.toLowerCase() === 'q') {
                    if (isValidQueenMove(row, col, kingPos.row, kingPos.col)) return true;
                } else if (piece.toLowerCase() === 'k') {
                    if (isValidKingMove(row, col, kingPos.row, kingPos.col,true)) return true;
                }
            }
        }
    }
...
function setPromotionPiece(element){
    const newPiece = element.value.toLowerCase();
    console.log(newPiece);
    console.log(board[promorow][promocol]);
    const select = document.getElementById('promotion-pick');
    select.style.display='none';
    if (['q', 'r', 'b', 'n'].includes(newPiece)) {
        board[promorow][promocol] = promopiece === 'P' ? newPiece.toUpperCase() : newPiece;
    } else {
        board[promorow][promocol] = promopiece === 'P' ? 'Q' : '';
    }
